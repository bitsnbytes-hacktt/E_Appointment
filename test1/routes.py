...
def send_register_email(new_user):
    ...
    msg.body = f'''
Welcome {new_user.name} to TTAppointments.
'''
    mail.send(msg)
...
def admin1():
    ...
    if request.method == 'POST':
        ...
        new_settings = Setting(start_time=a, end_time=b, time_period=e, limit=form.capacity.data, department_id=form.department.data)
        db.session.add(new_settings)
        db.session.commit()
        return redirect(url_for('admin2', start = a, end = b, period = e, departmentid = d))
        #return str(a) + "  " + str(b) 
...
def admin2(start,end,period,departmentid):
    ...
    for time in mylist:
        new_service = Service_Time(start_time=time, department_id=departmentid)
