...
def admin1():
    ...
    form.department.choices=[(department.id, department.name) for department in Department.query.filter_by(ministry_id=1).all()]
    form.location.choices=[(location.id, location.name) for location in Location.query.filter(Location.department_location.any(id=1)).all()]
    a= form.startTime.data
    b= form.endTime.data
    c=form.period.data
    d=form.department.data
    if request.method == 'POST':
        new_settings = Setting(start_time=form.startTime.data, end_time=form.endTime.data, time_period=form.period.data, limit=form.capacity.data, department_id=form.department.data)
        db.session.add(new_settings)
        db.session.commit()
       
        return redirect(url_for('admin2', start = a, end=b, period=c,departmentid=d))
    else:
        return render_template('adminSettings.html',form=form)

@app.route("/adminSettingsError/<start>/<end>/<period>/<departmentid>")
def admin2(start,end,period,departmentid):
    ...
    while x<en:
        timeObj={}
        timeObj['department_id']=departmentid
        timeObj['stime']=x
        timeArray.append(timeObj)
        x=x+prd
    for y in timeArray:
        new_service = Service_Time(start_time=y['stime'], department_id=departmentid1)
